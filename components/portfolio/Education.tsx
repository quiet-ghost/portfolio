...
export function Education() {
  ...
  const getStatusColor = (status: string) => {
    if (isDark) {
      ...
      switch (status) {
        case "completed": return "border-green-600/60 bg-green-100";
        case "current": return "border-cyan-600/60 bg-cyan-100 animate-pulse";
        case "in-progress": return "border-yellow-600/60 bg-yellow-100";
        case "upcoming": return "border-purple-600/60 bg-purple-100";
        default: return "border-cyan-600/60 bg-cyan-100";
      }
    }
  };
        ...
        <div className="relative">
          ...
          <div className="space-y-12">
            {educationData.map((item, index) => (
              ...
              >
                {/* Icon */}
                <div className={`absolute left-0 w-16 h-16 rounded-full border-2 ${getStatusColor(item.status)} flex items-center justify-center z-10 group-hover:scale-110 transition-transform duration-300 ${
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-3 mb-2">
                          ...
                          <span className={`px-3 py-1 rounded-full text-xs font-medium border ${
                            isDark 
                              ? item.status === 'completed' ? 'bg-green-400/20 text-green-400 border-green-400/40' :
                                item.status === 'current' ? 'bg-cyan-400/20 text-cyan-400 border-cyan-400/40' :
                                item.status === 'in-progress' ? 'bg-yellow-400/20 text-yellow-400 border-yellow-400/40' :
                                'bg-purple-400/20 text-purple-400 border-purple-400/40'
                              : item.status === 'completed' ? 'bg-green-100 text-green-800 border-green-300' :
                                item.status === 'current' ? 'bg-cyan-100 text-cyan-800 border-cyan-300' :
                                item.status === 'in-progress' ? 'bg-yellow-100 text-yellow-800 border-yellow-300' :
